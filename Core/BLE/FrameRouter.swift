        ...
        case "COMMAND_RESPONSE":
            ...
            if family == .whoop4, let cmd = parsed.cmdName {
                if cmd.hasPrefix("GET_ADVERTISING_NAME_HARVARD") {
                    ...
                    // armed rather than only what we sent. The decoded epoch (or nil for an
                    // unrecognised payload) also feeds the coordinator's verify loop via
                    // onAlarmReadback. That loop can only ever re-send the same arm or downgrade
                    // the status line; it can never cancel, re-arm to a different instant, or fire
                    // anything — the 4.0 reply layout is undocumented, so an unrecognised payload is
                    // logged as raw hex and counted as an unconfirmed attempt, nothing more. The
                    // wording says "strap reports", never "verified": verification is the
                    // coordinator's call once the echoed epoch matches what it sent.
                    if let epoch = Self.armedAlarmEpoch(in: frame) {
                        ...
                        state.onAlarmReadback?(epoch)
                    } else {
                        state.append(log: "Alarm: strap answered the alarm readback with an unrecognised payload (raw \(Self.commandResponsePayloadHex(in: frame) ?? "empty")) - layout undocumented")
                        state.onAlarmReadback?(nil)
                    }
                }
            }
