...
/// The strap-alarm surface the coordinator drives. The coordinator arms/disarms ONLY through
/// `armBackstop(at:)` / `dropBackstop()`, which BLEManager implements with ACKNOWLEDGED writes in the
/// extension below; the legacy `armStrapAlarm(at:)` / `disableStrapAlarm()` are the verbatim-frozen
/// BLEManager methods (`Core/BLE/BLEManager.swift`), kept as the defaults the acked pair forwards to so
/// every other conformer (the test fakes) keeps compiling. Abstracted only so tests can inject a fake;
/// the app passes the real `BLEManager`.
@MainActor
protocol AlarmStrap: AnyObject {
    /// Legacy UNACKED arm of the firmware alarm for `date`. BLEManager gates 5/MG behind Experimental.
    /// Not called by the coordinator; `armBackstop(at:)` is.
    func armStrapAlarm(at date: Date)
    /// Legacy UNACKED drop of the firmware alarm. Not called by the coordinator; `dropBackstop()` is.
    ...
    /// Arm the firmware backstop with ACKNOWLEDGED writes and request the readback. Default forwards to
    /// the legacy unacked `armStrapAlarm(at:)`; BLEManager overrides with `.withResponse` writes.
    func armBackstop(at date: Date)
    /// Drop the firmware backstop with an ACKNOWLEDGED write. Default forwards to `disableStrapAlarm()`.
    func dropBackstop()
    /// Whether `armBackstop(at:)` will ACTUALLY arm, rather than log a refusal and return. A bond alone
    ...
    /// Whether the strap answers `GET_ALARM_TIME` in a form the router parses (`onAlarmReadback`). False
    /// on a 5/MG: the router only decodes the reply for the WHOOP 4 family, so the verify loop would
    /// time out three times against an alarm that was in fact sent. Default true.
    var backstopVerifiable: Bool { get }
}

extension AlarmStrap {
    /// Defaults keep every existing conformer (the test fakes) compiling and behaving as before.
    var strapAlarmArmable: Bool { true }
    var backstopVerifiable: Bool { true }
    func armBackstop(at date: Date) { armStrapAlarm(at: date) }
    func dropBackstop() { disableStrapAlarm() }
}
...
extension BLEManager: AlarmStrap {
    ...
    /// Mirrors the family gate inside `armBackstop(at:)` (and the frozen `armStrapAlarm(at:)`) so the UI
    /// can tell "armed" from "refused". Read-only; changes nothing on the wire.
    var strapAlarmArmable: Bool { !isWhoop5 || PuffinExperiment.isEnabled }

    /// FrameRouter parses GET_ALARM_TIME only for the WHOOP 4 family, so a 5/MG arm can be sent but never
    /// echoed back.
    var backstopVerifiable: Bool { !isWhoop5 }

    /// The frozen `armStrapAlarm(at:)`, re-done with ACKNOWLEDGED writes. The unacked original was fired
    /// at the encrypted-bond edge (the least stable moment of the link) and the 2026-09-15 backup showed
    /// six days with zero STRAP_DRIVEN_ALARM_SET events despite three arms sent — the same dropped-write
    /// class as #921. Same family gate, same payloads, same clamp-not-trap epoch, same local-time log line;
    /// only the write type changes. On a WHOOP 4 the readback request follows, and the router feeds its
    /// reply to the coordinator's verify loop via `LiveState.onAlarmReadback`.
    func armBackstop(at date: Date) {
        // Log the wake time in the user's LOCAL zone: a bare date prints as UTC and reads like a
        // timezone bug. The command carries the absolute instant.
        let localFmt = DateFormatter()
        localFmt.dateFormat = "EEE HH:mm zzz"
        if isWhoop5 {
            guard PuffinExperiment.isEnabled else {
                state.append(log: "Alarm: 5/MG firmware alarm needs the Experimental toggle (unconfirmed) — not armed")
                return
            }
            let wakeMs = Int64((date.timeIntervalSince1970 * 1000).rounded())
            send(.setAlarmTime, payload: AlarmPayload.setAlarmRev4(wakeEpochMs: wakeMs), writeType: .withResponse)
            state.append(log: "Alarm: armed 5/MG rev4 (acked) for \(localFmt.string(from: date)) — your local wake time")
            return
        }
        ...
        state.append(log: "Alarm: armed (acked) for \(localFmt.string(from: date)) — your local wake time (sent as UTC epoch \(epochSec))")
        send(.getAlarmTime, payload: [0x01], writeType: .withResponse)
    }

    /// The frozen `disableStrapAlarm()` with an ACKNOWLEDGED write; same per-family body.
    func dropBackstop() {
        if isWhoop5 {
            send(.disableAlarm, payload: AlarmPayload.disableRev2(), writeType: .withResponse)
            state.append(log: "Alarm: disarmed (5/MG rev2, acked)")
            return
        }
        send(.disableAlarm, payload: [0x01], writeType: .withResponse)
        state.append(log: "Alarm: disarmed (acked)")
    }
}
...
///  1. FIRMWARE BACKSTOP — `armBackstop(at:)` at the window's LATEST edge, armed only over a genuine
///     encrypted bond, with ACKNOWLEDGED writes, then VERIFIED: the strap must echo the armed epoch back
///     (`onReadback`) before the status reads "armed", with up to `maxArmAttempts` attempts (re-sending
///     the same arm) on a missing/mismatched echo. Kill-proof once armed: the strap buzzes even if whoopmaxx is
///     closed, and the alarm survives strap BLE resets.
...
final class SmartAlarmCoordinator: ObservableObject {
    ...
    /// Layer-1 status as the Rest screen should report it. `.confirming` = arm sent, waiting for the
    /// strap's readback; `.verified` = the strap echoed the armed epoch; `.failed` = no matching echo after
    /// `maxArmAttempts` (a later matching echo still promotes to `.verified`); `.none` = nothing verified
    /// (unbonded, not armable, or a family with no readback).
    enum BackstopState: Equatable { case none, confirming, verified, failed }

    static let maxArmAttempts = 3
    static let readbackTimeout: TimeInterval = 5
    /// Delay before attempt 2, before attempt 3.
    static let retryBackoff: [TimeInterval] = [3, 6]

    let settings: SmartAlarmSettings

    @Published private(set) var backstop: BackstopState = .none
    ...
    /// Verify-loop identity. Every scheduled timeout/retry captures the value at scheduling time and is a
    /// no-op if it has moved on: a new `apply()`, `disarm()`, `earlyFire()`, `onStrapFired()`, or a
    /// failed attempt that already scheduled its retry, all bump it.
    private var generation = 0
    private var attempt = 0
    private var expectedEpoch: UInt32 = 0
    /// The instant the verify loop re-sends on retry — always the SAME arm, never a different one.
    private var verifyDeadline = Date.distantPast
    ...
    /// `schedule` is the verify loop's timer, injectable so tests drive it by hand.
    init(ble: AlarmStrap,
         live: LiveState,
    ...
    /// True only when the firmware backstop is verified armed on the strap right now (enabled + the strap
    /// echoed the armed epoch back). The Rest section's armed-status line — whose copy promises a buzz
    /// "even if whoopmaxx is closed" — reads `backstop` directly for the same rule, so a merely SENT arm
    /// reads as confirming/backup-only, never as armed; this property is the honesty tests' single seam.
    var isArmedOnStrap: Bool { settings.enabled && backstop == .verified }
    ...
    func apply() {
        ...
        // SET_ALARM_TIME characteristic, so arming there would silently no-op).
        if !live.encryptedBond {
            endVerify(.none)
            log?("Smart alarm: strap not bonded — backup notification only")
        } else if !ble.strapAlarmArmable {
            ...
            ble.armBackstop(at: deadline)
            endVerify(.none)
            log?("Smart alarm: backup notification only — the 5/MG firmware alarm needs the Experimental toggle")
        } else if !ble.backstopVerifiable {
            // The router cannot decode this family's readback, so the arm is sent (acked) but can never be
            // verified: no loop (it would only time out three times), and the status stays backup-only —
            // "armed" is claimed only on the strap's own echo, and the 5/MG wake has never been observed.
            ble.armBackstop(at: deadline)
            endVerify(.none)
            log?("Smart alarm: sent 5/MG backstop (acked; this family has no readback, so it is not reported as armed) — backup notification only")
        } else {
            startVerify(deadline)
        }
    ...
    /// Begin a fresh verify loop for `deadline`: attempt 1 goes out now, its timeout is scheduled, and the
    /// status is `.confirming` until the strap echoes `expectedEpoch` (`onReadback`).
    private func startVerify(_ deadline: Date) {
        generation += 1
        attempt = 0
        expectedEpoch = UInt32(clamping: Int64(deadline.timeIntervalSince1970))
        verifyDeadline = deadline
        backstop = .confirming
        sendAttempt(generation: generation)
    }

    /// Retire any in-flight loop (its pending timeouts/retries become no-ops) and settle the status.
    private func endVerify(_ state: BackstopState) {
        generation += 1
        backstop = state
    }

    private func sendAttempt(generation gen: Int) {
        guard gen == generation, backstop == .confirming else { return }
        attempt += 1
        ble.armBackstop(at: verifyDeadline)
        schedule(Self.readbackTimeout) { [weak self] in
            guard let self, gen == self.generation, self.backstop == .confirming else { return }
            self.log?("Smart alarm: no readback within \(Int(Self.readbackTimeout)) s (attempt \(self.attempt))")
            self.attemptFailed()
        }
    }

    /// The strap's answer to GET_ALARM_TIME (wired by AppRoot from `LiveState.onAlarmReadback`). A match
    /// verifies — also from `.failed`, since the echo carries no attempt number and a slow reply to the
    /// last attempt can land after the timeout gave up; the epoch is ground truth for the arm this loop
    /// sent, and accepting it can never cancel or re-arm anything. For the same reason a late nil/mismatch
    /// from attempt N can be charged to attempt N+1 and end the loop early; the next matching echo repairs
    /// that. A mismatch or an unrecognised reply counts as a failed attempt. Otherwise (nothing pending,
    /// already verified, or a late echo after a disarm/early fire) it is ignored.
    func onReadback(_ epoch: UInt32?) {
        if epoch == expectedEpoch, backstop == .confirming || backstop == .failed {
            backstop = .verified
            log?("Smart alarm: strap backstop armed and verified by readback (attempt \(attempt))")
            return
        }
        guard backstop == .confirming else { return }
        log?("Smart alarm: readback mismatch — strap reports \(epoch.map { String($0) } ?? "unrecognised"), expected \(expectedEpoch) (attempt \(attempt))")
        attemptFailed()
    }

    /// Re-send the SAME arm after the backoff, or give up after `maxArmAttempts`. This is the only thing
    /// the readback can ever cause: a re-send of the identical arm or a downgraded status line — never a
    /// cancel, a different instant, or a buzz.
    private func attemptFailed() {
        guard attempt < Self.maxArmAttempts else {
            backstop = .failed
            log?("Smart alarm: strap backstop NOT confirmed after \(Self.maxArmAttempts) attempts — backup notification only")
            return
        }
        let delay = Self.retryBackoff[attempt - 1]
        generation += 1   // retire the failed attempt's timeout so it can't schedule a second retry
        let gen = generation
        log?("Smart alarm: re-sending strap arm in \(Int(delay)) s (attempt \(attempt + 1) of \(Self.maxArmAttempts))")
        schedule(delay) { [weak self] in self?.sendAttempt(generation: gen) }
    }
    ...
    private func earlyFire() {
        ...
        ble.dropBackstop()           // detection advances the wake — drop the latest-edge backstop (acked)
        endVerify(.none)             // and retire any verify loop still waiting on its readback
    ...
    func onStrapFired() {
        ...
        // would otherwise reopen this path for the SAME morning's stray fire — earlyFire's dropBackstop()
        // write is acknowledged but never read back, so the firmware can still fire (a drop racing the
        // latest edge, possibly minutes after the early fire — past any short wall-clock debounce). If
        ...
        settings.record(WakeEvent(firedEpoch: now().timeIntervalSince1970,
                                  ...
                                  encryptedBond: live.encryptedBond))
        notifier.postWakeNow()
        endVerify(.none)   // the fired alarm is spent; apply() below starts tomorrow's verify loop
