...
final class AppRoot: ObservableObject {
    ...
    init() {
        ...
        //  • The strap echoed its armed epoch (GET_ALARM_TIME readback) → the coordinator's verify loop
        //    compares it to the deadline it sent and flips `backstop` to .verified, or retries.
        live.onAlarmReadback = { [weak self] epoch in self?.alarm.onReadback(epoch) }
