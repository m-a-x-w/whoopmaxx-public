...
//
// `zonesJSON` is the verbatim HR-zone-percentage object from a WHOOP CSV import ("z1"…"z5"; the Android
// importer writes "zone1"…"zone5" for the same data — tolerate both so a cache moved between platforms
// still renders). Values are 0–100 percent of the workout's duration and may sum to less than 100.
//
// Ported verbatim from the original `Strand/Data/WorkoutZones.swift`.
enum WorkoutZones {
    ...
    /// Duration-weighted zone minutes across rows. APPROXIMATE: an on-device aggregate of the imported
