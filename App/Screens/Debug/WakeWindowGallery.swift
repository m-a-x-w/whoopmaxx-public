...
/// `AppShell`), so agents can screenshot every state — disabled (toggle + explainer), enabled with the
/// backup-only / confirming / armed status lines — light AND dark, without scrolling past the Last-night hero on the real
