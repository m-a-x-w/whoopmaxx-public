...
struct WakeWindowSection: View {
    ...
    /// True when the strap has echoed the armed epoch back (coordinator `backstop == .verified`), so the
    /// firmware backstop is actually armed. The simulator has no BLE, so this is false there and the
    /// section honestly reads "Backup notification only". Passed in by the RestScreen wrapper.
    var strapArmed: Bool
    /// True while the arm has been sent and the strap has not yet echoed it (`backstop == .confirming`).
    /// Ignored when `strapArmed` is true. Defaults false so previews/specimens compile unchanged.
    var strapConfirming: Bool = false
    ...
    private var armState: WakeArmState {
        ...
        if strapArmed { return .armedOnStrap }
        return strapConfirming ? .confirming : .backupOnly
    }
...
/// The four honest arm states the section presents. Copy is plain, exact, no exclamation. `confirming`
/// and `backupOnly` share the neutral ink treatment; only the strap's own echo earns "Armed".
enum WakeArmState {
    case off
    case backupOnly
    case confirming
    ...
    var statusLine: String {
        ...
        case .confirming:   return "Confirming on the strap…"
    ...
    var caption: String {
        ...
        case .off:
            return ""
        case .backupOnly:
            return "The strap hasn't confirmed an alarm — it isn't connected, or it didn't echo the alarm back. A backup notification will try to wake you at the latest time. Keep a backup alarm — a notification can be silenced by Focus or silent mode."
        case .confirming:
            return "The alarm has been sent to the strap and whoopmaxx is waiting for it to echo the time back. The backup notification is already scheduled. If the strap doesn't confirm, this line goes back to backup-only."
        case .armedOnStrap:
