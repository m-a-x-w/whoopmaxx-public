...
struct RestScreen: View {
    ...
    private func content(_ a: RestModel.Assembly) -> some View {
        ...
        return RestScreenContent(
            ...
            // honestly reads "Backup notification only". P7: the LiveState observation (`connected`) and
            // the coordinator's `backstop` live inside `WakeWindowArmed` so their churn re-renders only that small section,
