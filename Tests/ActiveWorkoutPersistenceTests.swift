...
/// Pure + `UserDefaults`-backed. Ported from the original's `ActiveWorkoutPersistenceTests`.
final class ActiveWorkoutPersistenceTests: XCTestCase {
    ...
    func testEncodeDecodeRoundTripsEveryField() {
        ...
        XCTAssertEqual(ActiveWorkoutPersistence.decode(ActiveWorkoutPersistence.encode(original)), original)
    }
    ...
    func testDecodeRejectsNonPositiveStart() {
        XCTAssertNil(ActiveWorkoutPersistence.decode(ActiveWorkoutPersistence.encode(snapshot(startSec: 0))))
    }
